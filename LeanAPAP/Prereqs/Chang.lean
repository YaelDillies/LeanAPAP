...
variable {G : Type _} [AddCommGroup G] [Fintype G] {f : G → ℂ} {η : ℝ} {ψ : AddChar G ℂ}
  {Δ : Finset (AddChar G ℂ)} {m : ℕ}

/-- The `η`-large spectrum of a function. -/
noncomputable def largeSpec (f : G → ℂ) (η : ℝ) : Finset (AddChar G ℂ) :=
  univ.filterₓ fun ψ => η * ‖f‖_[1] ≤ ‖dft f ψ‖

@[simp]
theorem mem_largeSpec : ψ ∈ largeSpec f η ↔ η * ‖f‖_[1] ≤ ‖dft f ψ‖ := by simp [largeSpec]

theorem largeSpec_anti (f : G → ℂ) : Antitone (largeSpec f) := fun η ν h ψ => by
  simp_rw [mem_largeSpec] <;> exact (mul_le_mul_of_nonneg_right h lpnorm_nonneg).trans

@[simp]
theorem largeSpec_zero_left (η : ℝ) : largeSpec (0 : G → ℂ) η = univ := by simp [largeSpec]

@[simp]
theorem largeSpec_zero_right (f : G → ℂ) : largeSpec f 0 = univ := by simp [largeSpec]

private noncomputable def α (f : G → ℂ) :=
  ‖f‖_[1] ^ 2 / ‖f‖_[2] ^ 2 / card G

theorem α_nonneg (f : G → ℂ) : 0 ≤ α f := by unfold α <;> positivity

theorem α_pos (hf : f ≠ 0) : 0 < α f := by unfold α <;> positivity

theorem α_le_one (f : G → ℂ) : α f ≤ 1 :=
  by
...
theorem general_hoelder (hη : 0 ≤ η) (ν : G → ℝ≥0) (hfν : ∀ x, f x ≠ 0 → 1 ≤ ν x)
  ...
  · simp
  choose c norm_c hc using fun γ => IsROrC.exists_norm_eq_hMul_self (dft f γ)
  have :=
    calc
  ...
  rw [rpow_nat_inv_pow_nat (sum_nonneg fun _ _ => _) hm, ← rpow_mul_nat_cast, one_sub_mul,
    inv_mul_cancel, ← Nat.cast_pred, rpow_nat_cast, mul_assoc, mul_left_comm, ← pow_sub_one_hMul,
  ...
  calc
    ...
    _ =
        ‖∑ γ in pi_finset fun i : Fin m => Δ,
            ∑ δ in pi_finset fun i : Fin m => Δ,
...
theorem spec_hoelder (hη : 0 ≤ η) (hΔ : Δ ⊆ largeSpec f η) (hm : m ≠ 0) :
    ↑Δ.card ^ (2 * m) * (η ^ (2 * m) * α f) ≤ boringEnergy m Δ :=
  by
...
/-- **Chang's lemma**. -/
theorem chang (hf : f ≠ 0) (hη : 0 < η) :
    ∃ (Δ : _) (_ : Δ ⊆ largeSpec f η),
      Δ.card ≤ thomasConst * ⌈exp 1 * ⌈curlog (α f)⌉₊ / η ^ 2⌉₊ ∧ largeSpec f η ⊆ Δ.addSpan :=
  by
  refine' diss_add_span fun Δ hΔη hΔ => _
