...
@[simp]
lemma translate_apply (a : α) (f : α → β) (x : α) : τ a f x = f (x - a) := rfl

@[simp]
lemma translate_zero (f : α → β) : translate 0 f = f := by ext <;> simp
...
@[simp]
lemma comp_translate (a : α) (f : α → β) (g : β → γ) : g ∘ τ a f = τ a (g ∘ f) := rfl
...
@[simp]
lemma translate_zero_right (a : α) : τ a (0 : α → β) = 0 := rfl
...
lemma sum_translate [Fintype α] (a : α) (f : α → β) : ∑ b, τ a f b = ∑ b, f b :=
  Fintype.sum_equiv (Equiv.subRight _) _ _ λ _ => rfl

@[simp]
lemma support_translate (a : α) (f : α → β) : support (τ a f) = a +ᵥ support f := by
...
@[simp]
lemma conjneg_apply (f : α → β) (x : α) : conjneg f x = conj (f (-x)) := rfl

@[simp]
lemma conjneg_conjneg (f : α → β) : conjneg (conjneg f) = f := by ext <;> simp

lemma conjneg_injective : Injective (conjneg : (α → β) → α → β) :=
  Involutive.injective conjneg_conjneg

@[simp]
lemma conjneg_inj : conjneg f = conjneg g ↔ f = g :=
  conjneg_injective.eq_iff

lemma conjneg_ne_conjneg : conjneg f ≠ conjneg g ↔ f ≠ g :=
  conjneg_injective.ne_iff

@[simp]
lemma conjneg_zero : conjneg (0 : α → β) = 0 := by ext <;> simp

@[simp]
lemma conjneg_one : conjneg (1 : α → β) = 1 := by ext <;> simp

@[simp]
lemma conjneg_add (f g : α → β) : conjneg (f + g) = conjneg f + conjneg g := by ext <;> simp

@[simp]
lemma conjneg_sum (s : Finset ι) (f : ι → α → β) :
    conjneg (∑ i in s, f i) = ∑ i in s, conjneg (f i) := by
  ext <;> simp only [map_sum, conjneg_apply, Finset.sum_apply]

@[simp]
lemma conjneg_prod (s : Finset ι) (f : ι → α → β) :
    conjneg (∏ i in s, f i) = ∏ i in s, conjneg (f i) := by
  ext <;> simp only [map_prod, conjneg_apply, Finset.prod_apply]

@[simp]
lemma conjneg_eq_zero : conjneg f = 0 ↔ f = 0 := by
  rw [← conjneg_inj, conjneg_conjneg, conjneg_zero]

@[simp]
lemma conjneg_eq_one : conjneg f = 1 ↔ f = 1 := by
  rw [← conjneg_inj, conjneg_conjneg, conjneg_one]

lemma conjneg_ne_zero : conjneg f ≠ 0 ↔ f ≠ 0 :=
  conjneg_eq_zero.Not

lemma conjneg_ne_one : conjneg f ≠ 1 ↔ f ≠ 1 :=
  conjneg_eq_one.Not
...
@[simp]
lemma support_conjneg (f : α → β) : support (conjneg f) = -support f := by
...
@[simp]
lemma conjneg_sub (f g : α → β) : conjneg (f - g) = conjneg f - conjneg g := by ext <;> simp

@[simp]
lemma conjneg_neg (f : α → β) : conjneg (-f) = -conjneg f := by ext <;> simp
...
@[simp]
lemma conjneg_nonneg : 0 ≤ conjneg f ↔ 0 ≤ f :=
  (Equiv.neg _).forall_congr' λ x => star_nonneg

@[simp]
lemma conjneg_pos : 0 < conjneg f ↔ 0 < f := by
...
@[simp]
lemma conjneg_nonpos : conjneg f ≤ 0 ↔ f ≤ 0 := by
...
protected lemma IsSelfAdjoint.dilate (hf : IsSelfAdjoint f) (n : ℕ) :
    IsSelfAdjoint (dilate f n) :=
  Pi.isSelfAdjoint.2 λ g => hf.apply _
