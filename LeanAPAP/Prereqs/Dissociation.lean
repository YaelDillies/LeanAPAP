...
instance {α : Type*} [Fintype α] [DecidableEq α] {s : Set α} [DecidablePred (· ∈ s)] :
...
def MulDissociated (A : Finset α) : Prop :=
  ∀ a n, ((A.powersetLen n).filterₓ λ A' => ∏ x in A', x = a : Set <| Finset α).Subsingleton
...
lemma not_mulDissociated :
    ¬MulDissociated A ↔
      ∃ a n, ((A.powersetLen n).filterₓ λ A' => ∏ x in A', x = a : Set <| Finset α).Nontrivial := by simp [mul_dissociated]
...
lemma not_mulDissociated_iff_exists_disjoint :
    ¬MulDissociated A ↔
      ∃ (B : _) (_ : B ⊆ A) (C : _) (_ : C ⊆ A),
        Disjoint B C ∧ B ≠ C ∧ B.card = C.card ∧ ∏ a in B, a = ∏ a in C, a := by
...
lemma mulDissociated_iff_forall_lt_card :
    MulDissociated A ↔
      ∀ (a),
        ∀ n < A.card,
          ((A.powersetLen n).filterₓ λ A' => ∏ x in A', x = a : Set <| Finset α).Subsingleton :=
...
def mulSpan (A : Finset α) : Finset α :=
  (Fintype.piFinset λ a => ({-1, 0, 1} : Finset ℤ)).image λ c => ∏ a in A, a ^ c a
...
lemma subset_addSpan {α : Type*} [DecidableEq α] [AddCommGroup α] {A : Finset α} [Fintype α] :
    A ⊆ addSpan A := λ a ha =>
  mem_add_span.2
    ⟨Pi.single a 1, λ b => by obtain rfl | hab := eq_or_ne a b <;> simp [*], by
...
lemma diss_mulSpan (hA : ∀ (A') (_ : A' ⊆ A), A'.MulDissociated → A'.card ≤ K) :
    ∃ (A' : _) (_ : A' ⊆ A), A'.card ≤ K ∧ A ⊆ mulSpan A' := by
  obtain ⟨A', hA', hA'max⟩ :=
  ...
  obtain ⟨B, hB, C, hC, hBC⟩ :=
    not_mul_dissociated_iff_exists_disjoint.1 λ h =>
