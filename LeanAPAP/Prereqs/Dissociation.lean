...
import Project.Mathlib.Algebra.GroupPower.Basic
import Project.Mathlib.Data.Finset.Powerset

#align_import prereqs.dissociation
...
instance {α : Type _} [Fintype α] [DecidableEq α] {s : Set α} [DecidablePred (· ∈ s)] :
...
variable {α : Type _} [DecidableEq α] [CommGroup α] {A B C : Finset α} {K : ℕ} {a : α}

@[to_additive]
def MulDissociated (A : Finset α) : Prop :=
  ∀ a n, ((A.powersetLen n).filterₓ fun A' => ∏ x in A', x = a : Set <| Finset α).Subsingleton

@[simp, to_additive]
theorem mulDissociated_empty : MulDissociated (∅ : Finset α) := by
  simp [mul_dissociated, Set.Subsingleton, mem_powerset_len, subset_empty]

@[simp, to_additive]
theorem mulDissociated_singleton (a : α) : MulDissociated ({a} : Finset α) :=
  by
  simp only [mul_dissociated, Set.Subsingleton, mem_powerset_len, eq_comm, coe_filter, and_true_iff,
    ...
    imp_true_iff]
  refine' fun b n => ⟨_, _⟩ <;> rintro rfl rfl <;> simp

@[simp, to_additive]
theorem not_mulDissociated :
    ¬MulDissociated A ↔
      ∃ a n, ((A.powersetLen n).filterₓ fun A' => ∏ x in A', x = a : Set <| Finset α).Nontrivial :=
  by simp [mul_dissociated]

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (B «expr ⊆ » A) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (C «expr ⊆ » A) -/
@[to_additive]
theorem not_mulDissociated_iff_exists_disjoint :
    ¬MulDissociated A ↔
      ∃ (B : _) (_ : B ⊆ A) (C : _) (_ : C ⊆ A),
        Disjoint B C ∧ B ≠ C ∧ B.card = C.card ∧ ∏ a in B, a = ∏ a in C, a :=
  by
  ...
  · rintro ⟨a, n, B, hB, C, hC, hBC⟩
    ...
    refine'
      ⟨B \ C, (sdiff_subset _ _).trans hB.1.1, C \ B, (sdiff_subset _ _).trans hC.1.1,
        disjoint_sdiff_sdiff, fun h => hBC _, add_left_injective _ <| card_sdiff_add_card.trans _,
        _⟩
...
@[to_additive]
theorem mulDissociated_iff_forall_lt_card :
    MulDissociated A ↔
      ∀ (a),
        ∀ n < A.card,
          ((A.powersetLen n).filterₓ fun A' => ∏ x in A', x = a : Set <| Finset α).Subsingleton :=
  forall₂_congr fun a n => by
    obtain hn | rfl | hn := lt_trichotomy n A.card <;> simp [*, Set.Subsingleton]

variable [Fintype α]

@[to_additive]
instance : DecidablePred (MulDissociated : Finset α → Prop) := fun A =>
  decidable_of_iff'
    (∀ (a),
      ∀ n ∈ range A.card,
        ((A.powersetLen n).filterₓ fun A' => ∏ x in A', x = a : Set <| Finset α).Subsingleton)
...
def mulSpan (A : Finset α) : Finset α :=
  (Fintype.piFinset fun a => ({-1, 0, 1} : Finset ℤ)).image fun c => ∏ a in A, a ^ c a
...
@[simp]
theorem subset_addSpan {α : Type _} [DecidableEq α] [AddCommGroup α] {A : Finset α} [Fintype α] :
    A ⊆ addSpan A := fun a ha =>
  mem_add_span.2
    ⟨Pi.single a 1, fun b => by obtain rfl | hab := eq_or_ne a b <;> simp [*], by
      simp [Pi.single, Function.update, ite_smul, ha]⟩

@[simp, to_additive]
theorem subset_mulSpan : A ⊆ mulSpan A := fun a ha =>
  mem_mulSpan.2
    ⟨Pi.single a 1, fun b => by obtain rfl | hab := eq_or_ne a b <;> simp [*], by
      simp [Pi.single, Function.update, pow_ite, ha]⟩

--TODO: Fix additivisation
theorem sum_sub_sum_mem_addSpan {α : Type _} [DecidableEq α] [AddCommGroup α] {A B C : Finset α}
...
@[to_additive]
theorem prod_div_prod_mem_mulSpan (hB : B ⊆ A) (hC : C ⊆ A) :
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (A' «expr ⊆ » A) -/
@[to_additive]
theorem diss_mulSpan (hA : ∀ (A') (_ : A' ⊆ A), A'.MulDissociated → A'.card ≤ K) :
    ∃ (A' : _) (_ : A' ⊆ A), A'.card ≤ K ∧ A ⊆ mulSpan A' :=
  by
  obtain ⟨A', hA', hA'max⟩ :=
    exists_maximal (A.powerset.filter mul_dissociated)
  ...
  obtain ⟨B, hB, C, hC, hBC⟩ :=
    not_mul_dissociated_iff_exists_disjoint.1 fun h =>
      hA'max _ (insert_subset.2 ⟨ha, hA'.1⟩) h <| ssubset_insert ha'
  by_cases haB : a ∈ B
  · have : a = (∏ b in C, b) / ∏ b in B.erase a, b := by
      rw [prod_erase_eq_div haB, hBC.2.2.2, div_div_self']
    rw [this]
    exact
      prod_div_prod_mem_mul_span ((subset_insert_iff_of_not_mem <| disjoint_left.1 hBC.1 haB).1 hC)
