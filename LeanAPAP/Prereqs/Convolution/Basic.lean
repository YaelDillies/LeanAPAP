...
/-- Convolution -/ @[ nolint unused_arguments ]
  def
    Function.conv
    ( f g : α → β ) : α → β
    := λ a => ∑ x : α × α with x . 1 + x . 2 = a , f x . 1 * g x . 2
...
/-- Difference convolution -/
  def
    dconv
    ( f g : α → β ) : α → β
    := λ a => ∑ x : α × α with x . 1 - x . 2 = a , f x . 1 * conj g x . 2
...
@[simp]
lemma trivChar_apply (a : α) : (trivChar a : β) = if a = 0 then 1 else 0 := rfl

@[simp]
lemma conv_conjneg (f g : α → β) : f ∗ conjneg g = f ○ g :=
  funext λ a =>
    sum_bij (λ x _ => (x.1, -x.2)) (λ x hx => by simpa using hx) (λ x _ => rfl)
      (λ x y _ _ h => by simpa [Prod.ext_iff] using h) λ x hx =>
      ⟨(x.1, -x.2), by simpa [sub_eq_add_neg] using hx, by simp⟩

@[simp]
lemma dconv_conjneg (f g : α → β) : f ○ conjneg g = f ∗ g := by
  rw [← conv_conjneg, conjneg_conjneg]

lemma conv_comm (f g : α → β) : f ∗ g = g ∗ f :=
  funext λ a =>
    sum_nbij' Prod.swap (λ x hx => by simpa [add_comm] using hx) (λ x _ => mul_comm _ _)
      Prod.swap (λ x hx => by simpa [add_comm] using hx) (λ x _ => x.swap_swap) λ x _ =>
      x.swap_swap

@[simp]
lemma conj_conv (f g : α → β) : conj (f ∗ g) = conj f ∗ conj g :=
  funext λ a => by simp only [Pi.conj_apply, conv_apply, map_sum, map_mul]

@[simp]
lemma conjneg_conv (f g : α → β) : conjneg (f ∗ g) = conjneg f ∗ conjneg g := by
  funext a
...
@[simp]
lemma conjneg_dconv (f g : α → β) : conjneg (f ○ g) = g ○ f := by
...
lemma conv_assoc (f g h : α → β) : f ∗ g ∗ h = f ∗ (g ∗ h) := by
  ...
  refine'
          sum_bij' (λ x hx => ⟨(x.2.1, x.2.2 + x.1.2), (x.2.2, x.1.2)⟩) _ _
...
@[simp]
lemma conv_zero (f : α → β) : f ∗ 0 = 0 := by ext <;> simp [conv_apply]

@[simp]
lemma zero_conv (f : α → β) : 0 ∗ f = 0 := by ext <;> simp [conv_apply]

@[simp]
lemma dconv_zero (f : α → β) : f ○ 0 = 0 := by simp [← conv_conjneg]

@[simp]
lemma zero_dconv (f : α → β) : 0 ○ f = 0 := by simp [← conv_conjneg]
...
lemma map_dconv (f g : α → ℝ≥0) (a : α) : (↑((f ○ g) a) : ℝ) = (coe ∘ f ○ coe ∘ g) a := by
...
lemma conv_eq_sum_sub (f g : α → β) (a : α) : (f ∗ g) a = ∑ t, f (a - t) * g t := by
  rw [conv_apply]
  refine'
      sum_bij (λ x _ => x.2) (λ x _ => mem_univ _) _ _ λ b _ =>
...
lemma conv_eq_sum_add (f g : α → β) (a : α) : (f ∗ g) a = ∑ t, f (a + t) * g (-t) :=
  (conv_eq_sum_sub _ _ _).trans <|
    Fintype.sum_equiv (Equiv.neg _) _ _ λ t => by
...
lemma conv_apply_add (f g : α → β) (a b : α) : (f ∗ g) (a + b) = ∑ t, f (a + t) * g (b - t) :=
  (conv_eq_sum_sub _ _ _).trans <|
    Fintype.sum_equiv (Equiv.subLeft b) _ _ λ t => by simp [add_sub_assoc, ← sub_add]
...
lemma sum_dconv_mul (f g h : α → β) :
  ...
  rw [sum_comm]
  exact sum_congr rfl λ x _ => Fintype.sum_equiv (Equiv.subLeft x) _ _ λ y => by simp
...
@[simp]
lemma conv_const (f : α → β) (b : β) : f ∗ const _ b = const _ ((∑ x, f x) * b) := by
  ext <;> simp [conv_eq_sum_sub', sum_mul]

@[simp]
lemma const_conv (b : β) (f : α → β) : const _ b ∗ f = const _ (b * ∑ x, f x) := by
  ext <;> simp [conv_eq_sum_sub, mul_sum]

@[simp]
lemma dconv_const (f : α → β) (b : β) : f ○ const _ b = const _ ((∑ x, f x) * conj b) := by
  ext <;> simp [dconv_eq_sum_sub, sum_mul]

@[simp]
lemma const_dconv (b : β) (f : α → β) : const _ b ○ f = const _ (b * ∑ x, conj (f x)) := by
  ext <;> simp [dconv_eq_sum_add, mul_sum]

@[simp]
lemma conv_trivChar (f : α → β) : f ∗ trivChar = f := by ext a; simp [conv_eq_sum_sub]

@[simp]
lemma trivChar_conv (f : α → β) : trivChar ∗ f = f := by rw [conv_comm, conv_trivChar]

@[simp]
lemma dconv_trivChar (f : α → β) : f ○ trivChar = f := by ext a; simp [dconv_eq_sum_add]

@[simp]
lemma trivChar_dconv (f : α → β) : trivChar ○ f = conjneg f := by
...
lemma indicate_conv_indicate_apply (s t : Finset α) (a : α) :
    (𝟭_[β] s ∗ 𝟭 t) a = ((s ×ˢ t).filterₓ λ x : α × α => x.1 + x.2 = a).card := by
...
@[simp]
lemma conv_neg (f g : α → β) : f ∗ -g = -(f ∗ g) := by ext <;> simp [conv_apply]

@[simp]
lemma neg_conv (f g : α → β) : -f ∗ g = -(f ∗ g) := by ext <;> simp [conv_apply]

@[simp]
lemma dconv_neg (f g : α → β) : f ○ -g = -(f ○ g) := by ext <;> simp [dconv_apply]

@[simp]
lemma neg_dconv (f g : α → β) : -f ○ g = -(f ○ g) := by ext <;> simp [dconv_apply]
...
@[simp]
lemma mu_univ_conv_mu_univ : μ_[β] (univ : Finset α) ∗ μ univ = μ univ := by
  ext <;> cases eq_or_ne (card α : β) 0 <;> simp [mu_apply, conv_eq_sum_add, card_univ, *]

@[simp]
lemma mu_univ_dconv_mu_univ : μ_[β] (univ : Finset α) ○ μ univ = μ univ := by
...
@[simp]
lemma balance_conv (f g : α → β) : balance (f ∗ g) = balance f ∗ balance g := by
  simp [balance, conv_sub, sub_conv, expect_conv]

@[simp]
lemma balance_dconv (f g : α → β) : balance (f ○ g) = balance f ○ balance g := by
...
@[simp, norm_cast]
lemma coe_conv : (↑((f ∗ g) a) : 𝕜) = (coe ∘ f ∗ coe ∘ g) a :=
  map_conv (algebraMap ℝ 𝕜) _ _ _

@[simp, norm_cast]
lemma coe_dconv : (↑((f ○ g) a) : 𝕜) = (coe ∘ f ○ coe ∘ g) a := by simp [dconv_apply, coe_sum]

@[simp]
lemma coe_comp_conv : (coe : ℝ → 𝕜) ∘ (f ∗ g) = coe ∘ f ∗ coe ∘ g :=
  funext <| coe_conv _ _

@[simp]
lemma coe_comp_dconv : (coe : ℝ → 𝕜) ∘ (f ○ g) = coe ∘ f ○ coe ∘ g :=
...
@[simp, norm_cast]
lemma coe_conv : (↑((f ∗ g) a) : ℝ) = (coe ∘ f ∗ coe ∘ g) a :=
  map_conv NNReal.toRealHom _ _ _

@[simp, norm_cast]
lemma coe_dconv : (↑((f ○ g) a) : ℝ) = (coe ∘ f ○ coe ∘ g) a := by simp [dconv_apply, coe_sum]

@[simp]
lemma coe_comp_conv : (coe : _ → ℝ) ∘ (f ∗ g) = coe ∘ f ∗ coe ∘ g :=
  funext <| coe_conv _ _

@[simp]
lemma coe_comp_dconv : (coe : _ → ℝ) ∘ (f ○ g) = coe ∘ f ○ coe ∘ g :=
  funext <| coe_dconv _ _
...
@[simp]
lemma iterConv_zero (f : α → β) : f ∗^ 0 = trivChar := rfl

@[simp]
lemma iterConv_one (f : α → β) : f ∗^ 1 = f :=
  conv_trivChar _

lemma iterConv_succ (f : α → β) (n : ℕ) : f ∗^ (n + 1) = f ∗ f ∗^ n := rfl

lemma iterConv_succ' (f : α → β) (n : ℕ) : f ∗^ (n + 1) = f ∗^ n ∗ f :=
  conv_comm _ _
...
@[simp]
lemma conj_iterConv (f : α → β) : ∀ n, conj (f ∗^ n) = conj f ∗^ n
  | 0 => by ext <;> simp
...
@[simp]
lemma zero_iterConv : ∀ {n}, n ≠ 0 → (0 : α → β) ∗^ n = 0
  | 0, hn => by cases hn rfl
  | n + 1, _ => zero_conv _

@[simp]
lemma smul_iterConv [Monoid γ] [DistribMulAction γ β] [IsScalarTower γ β β] [SMulCommClass γ β β]
    (c : γ) (f : α → β) : ∀ n, (c • f) ∗^ n = c ^ n • f ∗^ n
  | 0 => by simp
  | n + 1 => by simp_rw [iterConv_succ, smul_iterConv n, pow_succ, mul_smul_conv_comm]
...
@[simp]
lemma iterConv_trivChar : ∀ n, (trivChar : α → β) ∗^ n = trivChar
  | 0 => rfl
...
lemma indicate_iterConv_apply (s : Finset α) (n : ℕ) (a : α) :
    (𝟭_[ℝ] s ∗^ n) a = ((piFinset λ i => s).filterₓ λ x : Fin n → α => ∑ i, x i = a).card := by
...
@[simp]
lemma balance_iterConv (f : α → β) : ∀ {n}, n ≠ 0 → balance (f ∗^ n) = balance f ∗^ n
  | 0, h => by cases h rfl
  | 1, h => by simp
...
@[simp, norm_cast]
lemma coe_iterConv (f : α → ℝ≥0) (n : ℕ) (a : α) : (↑((f ∗^ n) a) : ℝ) = (coe ∘ f ∗^ n) a :=
