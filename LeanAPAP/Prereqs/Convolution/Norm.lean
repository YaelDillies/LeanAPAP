...
#align_import prereqs.convolution.norm
...
theorem conv_eq_inner (f g : α → β) (a : α) : (f ∗ g) a = ⟪conj f, τ a fun x => g (-x)⟫_[β] := by
  simp [l2inner_eq_sum, conv_eq_sum_sub', map_sum]

theorem dconv_eq_inner (f g : α → β) (a : α) : (f ○ g) a = conj ⟪f, τ a g⟫_[β] := by
...
theorem lpnorm_conv_le {p : ℝ≥0} (hp : 1 ≤ p) (f g : α → β) : ‖f ∗ g‖_[p] ≤ ‖f‖_[p] * ‖g‖_[1] :=
  by
  ...
  have :
    ...
    rotate_left
    · positivity
    · exact sum_nonneg fun _ _ => norm_nonneg _
    · exact sum_nonneg fun _ _ => by positivity
    · exact rpow_nonneg (sum_nonneg fun _ _ => norm_nonneg _)
    ·
      exact
        mul_nonneg (sum_nonneg fun _ _ => by positivity)
          (rpow_nonneg <| sum_nonneg fun _ _ => norm_nonneg _)
    · positivity
    calc
...
/-- A special case of **Young's convolution inequality**. -/
theorem lpnorm_dconv_le {p : ℝ≥0} (hp : 1 ≤ p) (f g : α → β) : ‖f ○ g‖_[p] ≤ ‖f‖_[p] * ‖g‖_[1] := by
...
--TODO: Include `f : α → ℂ`
theorem L1norm_conv (hf : 0 ≤ f) (hg : 0 ≤ g) : ‖f ∗ g‖_[1] = ‖f‖_[1] * ‖g‖_[1] :=
  by
...
theorem L1norm_dconv (hf : 0 ≤ f) (hg : 0 ≤ g) : ‖f ○ g‖_[1] = ‖f‖_[1] * ‖g‖_[1] := by
