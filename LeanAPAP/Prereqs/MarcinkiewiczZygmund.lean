...
private lemma step_two_aux (A : Finset G) (f : G → ℝ) (ε : Fin n → ℝ)
    ...
    ∑ (a) (b) in A^^n, g (ε * (f ∘ a - f ∘ b)) = ∑ (a) (b) in A^^n, g (f ∘ a - f ∘ b) := by
  ...
  have h₁ : ∀ a ∈ (A^^n) ×ˢ (A^^n), swapper a ∈ (A^^n) ×ˢ (A^^n) := by
    ...
    · exact h i
    exact (h i).symm
  have h₂ : ∀ a ∈ (A^^n) ×ˢ (A^^n), swapper (swapper a) = a := by
    intro a ha
    ext
...
private lemma step_two (hA : A.Nonempty) (f : G → ℝ) :
    ∑ (a) (b) in A^^n, (∑ i, (f (a i) - f (b i))) ^ (2 * m) =
      1 / 2 ^ n *
        ∑ ε in ({-1, 1} : Finset ℝ)^^n,
          ∑ (a) (b) in A^^n, (∑ i, ε i * (f (a i) - f (b i))) ^ (2 * m) := by
  have :
    ∀ ε : Fin n → ℝ,
      ε ∈ ({-1, 1} : Finset ℝ)^^n →
        ∑ (a) (b) in A^^n, (∑ i, ε i * (f (a i) - f (b i))) ^ (2 * m) =
          ∑ (a) (b) in A^^n, (∑ i, (f (a i) - f (b i))) ^ (2 * m) := by
    intro ε hε
...
lemma basic_marcinkiewicz_zygmund (f : G → ℝ) (hf : ∀ i : Fin n, ∑ a in A^^n, f (a i) = 0) :
    ∑ a in A^^n, |∑ i, f (a i)| ^ (2 * m) ≤ (4 * m) ^ m * ∑ a in A^^n, (∑ i, |f (a i)| ^ 2) ^ m := by
...
lemma other_marcinkiewicz_zygmund' (f : G → ℝ) (hm : m ≠ 0)
    (hf : ∀ i : Fin n, ∑ a in A^^n, f (a i) = 0) :
    ∑ a in A^^n, (∑ i, f (a i)) ^ (2 * m) ≤
      (4 * m) ^ m * n ^ (m - 1) * ∑ a in A^^n, ∑ i, f (a i) ^ (2 * m) := by simpa only [pow_mul, sq_abs] using other_marcinkiewicz_zygmund f hm hf
