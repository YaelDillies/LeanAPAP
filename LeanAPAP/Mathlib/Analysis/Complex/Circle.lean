...
@[simp, norm_cast]
lemma coe_exp (r : ℝ) : ↑(expMapCircle r) = Complex.exp (r * Complex.I) := rfl
...
noncomputable def e : AddChar ℝ circle :=
  AddChar.toMonoidHom'.symm
    { toλ := λ r => expMapCircle (2 * π * r.toAdd)
      map_one' := by simp
...
@[simp, norm_cast]
lemma coe_e (r : ℝ) : ↑(e r) = Complex.exp ((2 * π * r : ℝ) * Complex.I) := rfl

@[simp]
lemma e_int (z : ℤ) : e z = 1 :=
  exp_two_pi_mul_int _

@[simp]
lemma e_one : e 1 = 1 := by simpa using e_int 1

@[simp]
lemma e_add_int {z : ℤ} : e (r + z) = e r := by rw [map_add_mul, e_int, mul_one]

@[simp]
lemma e_sub_int {z : ℤ} : e (r - z) = e r := by rw [map_sub_eq_div, e_int, div_one]

@[simp]
lemma e_fract (r : ℝ) : e (Int.fract r) = e r := by rw [Int.fract, e_sub_int]

@[simp]
lemma e_mod_div {m : ℤ} {n : ℕ} : e ((m % n : ℤ) / n) = e (m / n) := by
