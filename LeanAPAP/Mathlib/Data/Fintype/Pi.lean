...
lemma piFinset_image (f : ∀ a, β a → γ a) (s : ∀ a, Finset (β a)) :
    (piFinset λ a ↦ image (f a) (s a)) =
      image (λ (b : ∀ a, β a) a ↦ f _ (b a)) (piFinset s) := by ext; simp only [mem_piFinset, mem_image, Classical.skolem, forall_and, Function.funext_iff]
...
@[reducible]
def piFinsetConst (s : Finset α) (n : ℕ) := piFinset λ _ : Fin n ↦ s
...
lemma image_piFinset_const [DecidableEq β] (t : Finset β) (a : α) :
    ((piFinset λ _i : α ↦ t).image λ f ↦ f a) = t := by
  obtain rfl | ht := t.eq_empty_or_nonempty
  · haveI : Nonempty α := ⟨a⟩
    simp
  · exact image_piFinset (λ _ ↦ t) a λ _ _ ↦ ht

lemma filter_piFinset_of_not_mem [∀ a, DecidableEq (δ a)] (t : ∀ a, Finset (δ a)) (a : α)
    (x : δ a) (hx : x ∉ t a) : ((piFinset t).filter λ f : ∀ a, δ a ↦ f a = x) = ∅ := by
...
lemma filter_piFinset_const_of_not_mem [DecidableEq β] (t : Finset β) (a : α) (x : β)
    (hx : x ∉ t) : ((piFinset λ _ ↦ t).filter λ f : α → β ↦ f a = x) = ∅ :=
