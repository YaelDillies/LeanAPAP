...
@[simp]
lemma zmodAux_apply (n : ℕ) (z : ℤ) : zmodAux n z = e (z / n) := by simp [zmod_aux]
...
@[simp]
lemma zMod_apply (n : ℕ) (x y : ℤ) : AddChar.zmod n x y = e (x * y / n) := by
...
lemma zMod_injective (hn : n ≠ 0) : Injective (AddChar.zmod n) := λ x y h => by
  have := FunLike.congr_λ h (1 : ZMod n) <;> simpa using zmod_aux_injective hn this

@[simp]
lemma zMod_inj (hn : n ≠ 0) {x y : ZMod n} : AddChar.zmod n x = AddChar.zmod n y ↔ x = y :=
  (zMod_injective hn).eq_iff

def zmodHom (n : ℕ) : AddChar (ZMod n) (AddChar (ZMod n) circle) :=
  AddChar.toMonoidHom'.symm
    { toλ := λ x => AddChar.toMonoidHom' (AddChar.zmod n x.toAdd)
      map_one' :=
        FunLike.ext _ _
          (by
...
def mkZmodAux {ι : Type} [DecidableEq ι] (n : ι → ℕ) (u : ∀ i, ZMod (n i)) :
  ...
  AddChar.directSum λ i => AddChar.zmod (n i) (u i)

lemma mkZmodAux_injective {ι : Type} [DecidableEq ι] {n : ι → ℕ} (hn : ∀ i, n i ≠ 0) :
    Injective (mkZmodAux n) :=
  AddChar.directSum_injective.comp λ f g h => by simpa [Function.funext_iff, hn] using h
...
def circleEquivComplex [Finite α] : AddChar α circle ≃+ AddChar α ℂ
    where
  toλ := circle.Subtype.comp
  invλ ψ :=
    AddChar.toMonoidHom'.symm
      { toλ := λ a => (⟨ψ a.toAdd, mem_circle_iff_abs.2 <| ψ.norm_apply _⟩ : circle)
        map_one' := by simp
...
@[simp]
lemma zmodAddEquiv_apply [NeZero n] (x : ZMod n) :
    AddChar.zmodAddEquiv (NeZero.ne _) x = circleEquivComplex (AddChar.zmod n x) := rfl
...
@[simp, norm_cast]
lemma coe_complexBasis : ⇑(complexBasis α) = coeFn := by
  rw [complex_basis, coe_basisOfLinearIndependentOfCardEqFinrank]

variable {α}

@[simp]
lemma complexBasis_apply (ψ : AddChar α ℂ) : complexBasis α ψ = ψ := by rw [coe_complex_basis]
...
lemma exists_apply_ne_zero : (∃ ψ : AddChar α ℂ, ψ a ≠ 1) ↔ a ≠ 0 := by
  ...
  have h₀ := congr_λ ((complex_basis α).sum_repr f) 0
  have h₁ := congr_λ ((complex_basis α).sum_repr f) a
...
lemma doubleDualEmb_injective : Injective (doubleDualEmb : α → AddChar (AddChar α ℂ) ℂ) :=
  injective_iff.2 λ a ha =>
    forall_apply_eq_zero.1 λ ψ => by simpa using FunLike.congr_λ ha (Additive.ofMul ψ)
...
@[simp]
lemma doubleDualEmb_eq_zero : (doubleDualEmb a : AddChar (AddChar α ℂ) ℂ) = 0 ↔ a = 0 := by
...
@[simp]
lemma doubleDualEmb_doubleDualEquiv_symm_apply (a : AddChar (AddChar α ℂ) ℂ) :
    doubleDualEmb (doubleDualEquiv.symm a) = a :=
  doubleDualEquiv.apply_symm_apply _

@[simp]
lemma doubleDualEquiv_symm_doubleDualEmb_apply (a : AddChar (AddChar α ℂ) ℂ) :
