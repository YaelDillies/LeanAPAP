...
@[simp]
lemma star_nonpos : star x ≤ 0 ↔ x ≤ 0 := by simp_rw [← neg_nonneg, ← star_neg, star_nonneg]

@[simp]
lemma star_neg' : star x < 0 ↔ x < 0 := by simp_rw [← neg_pos, ← star_neg, star_pos]
