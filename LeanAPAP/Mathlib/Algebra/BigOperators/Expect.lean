...
@[simp]
lemma expect_empty (f : α → 𝕜) : expect ∅ f = 0 := by simp [expect]

@[simp]
lemma expect_singleton (f : α → 𝕜) (a : α) : expect {a} f = f a := by simp [expect]

@[simp]
lemma expect_const_zero (s : Finset α) : 𝔼 x in s, (0 : 𝕜) = 0 := by simp [expect]
...
lemma expect_mul (s : Finset α) (f : α → 𝕜) (x : 𝕜) : (𝔼 i in s, f i) * x = 𝔼 i in s, f i * x := by rw [expect, div_mul_eq_mul_div, sum_mul]; rfl

lemma mul_expect (s : Finset α) (f : α → 𝕜) (x : 𝕜) : x * 𝔼 i in s, f i = 𝔼 i in s, x * f i := by
...
lemma expect_bij (i : ∀ a ∈ s, β) (hi : ∀ a ha, i a ha ∈ t) (h : ∀ a ha, f a = g (i a ha))
    ...
    (i_surj : ∀ b ∈ t, ∃ a ha, b = i a ha) : 𝔼 x in s, f x = 𝔼 x in t, g x := by
...
lemma expect_bij' (i : ∀ a ∈ s, β) (hi : ∀ a ha, i a ha ∈ t) (h : ∀ a ha, f a = g (i a ha))
    ...
    (right_inv : ∀ a ha, i (j a ha) (hj a ha) = a) : 𝔼 x in s, f x = 𝔼 x in t, g x := by
...
lemma expect_product' (f : α → β → 𝕜) : 𝔼 x in s ×ˢ t, f x.1 x.2 = 𝔼 x in s, 𝔼 y in t, f x y := by
...
lemma expect_multiplicative {G : Type*} [Fintype G] {f : Multiplicative G → 𝕜} :
    𝔼 x : Multiplicative G, f x = 𝔼 x, f (Multiplicative.ofAdd x) := rfl
...
@[simp]
lemma card_smul_expect (s : Finset α) (f : α → 𝕜) : s.card • 𝔼 i in s, f i = ∑ i in s, f i := by
...
@[simp]
lemma card_mul_expect (s : Finset α) (f : α → 𝕜) : ↑s.card * 𝔼 i in s, f i = ∑ i in s, f i := by
  rw [← nsmul_eq_mul, card_smul_expect]

@[simp]
lemma Fintype.card_smul_expect [Fintype α] (f : α → 𝕜) : Fintype.card α • 𝔼 i, f i = ∑ i, f i :=
  card_smul_expect _ _

@[simp]
lemma Fintype.card_mul_expect [Fintype α] (f : α → 𝕜) :
    ↑(Fintype.card α) * 𝔼 i, f i = ∑ i, f i :=
  card_mul_expect _ _

@[simp]
lemma expect_const (hs : s.Nonempty) (b : 𝕜) : 𝔼 i in s, b = b := by
...
@[simp]
lemma balance_zero : balance (0 : α → 𝕜) = 0 := by simp [balance]

@[simp]
lemma balance_add (f g : α → 𝕜) : balance (f + g) = balance f + balance g := by
...
@[simp, norm_cast]
lemma coe_balance : (↑(balance f a) : 𝕜) = balance (coe ∘ f) a :=
  map_balance (algebraMap ℝ 𝕜) _ _

@[simp]
lemma coe_comp_balance : (coe : ℝ → 𝕜) ∘ balance f = balance (coe ∘ f) :=
