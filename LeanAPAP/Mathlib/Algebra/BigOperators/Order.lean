...
#align_import mathlib.algebra.big_operators.order

open scoped BigOperators

namespace Finset

variable {α 𝕜 : Type _} [LinearOrderedCommRing 𝕜]

theorem sum_hMul_sq_le_sq_hMul_sq (s : Finset α) (f g : α → 𝕜) :
  ...
  simp_rw [sub_sq, sum_add_distrib, Finset.sum_sub_distrib, mul_pow, mul_assoc, ← mul_sum, ←
    ...
    two_mul, add_sub_cancel, mul_comm (∑ j in s, g j ^ 2), sq (∑ j in s, g j ^ 2), ← mul_assoc, ←
    mul_sub_right_distrib] at h 
  obtain h' | h' := (sum_nonneg fun i (hi : i ∈ s) => sq_nonneg (g i)).eq_or_lt
