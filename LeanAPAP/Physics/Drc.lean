...
import Project.Mathlib.Data.Finset.Pointwise
import Project.Mathlib.Data.Fintype.Lattice
import Project.Mathlib.Data.Real.Sqrt
import Project.Mathlib.Tactic.Positivity
...
variable {ι M : Type _} [OrderedCancelAddCommMonoid M] [Fintype ι] {f : ι → M}

open scoped BigOperators

theorem sum_pos (hf : 0 < f) : 0 < ∑ x, f x :=
  (Finset.sum_pos' fun _ _ => hf.le _) <| (Pi.lt_def.1 hf).2.imp <| by simp

end Fintype

namespace Function

variable {α R M : Type _} {n : ℕ}
...
@[simp]
theorem support_pow [MonoidWithZero M] [NoZeroDivisors M] (f : α → M) (hn : n ≠ 0) :
...
variable {G : Type _} [DecidableEq G] [Fintype G] [AddCommGroup G] {p : ℕ} {B₁ B₂ A : Finset G}
  {ε δ : ℝ}
...
private def C (p : ℕ) (A : Finset G) (s : Fin p → G) : Finset G :=
  univ.inf fun i => s i +ᵥ A

private theorem lemma_0 (p : ℕ) (B₁ B₂ A : Finset G) (f : G → ℝ) :
  ...
  simp only [l2inner_eq_sum, IsROrC.conj_to_real, mul_sum, sum_mul, smul_sum,
    @sum_comm _ _ (Fin p → G), sum_dconv_hMul, dconv_apply_sub, Fintype.sum_pow, map_indicate]
  congr with b₁
  congr with b₂
...
private theorem sum_C (p : ℕ) (B A : Finset G) : ∑ s, (B ∩ c p A s).card = A.card ^ p * B.card :=
  by
...
private theorem sum_cast_C (p : ℕ) (B A : Finset G) :
...
`𝟭 A ○ 𝟭 A` is positive. -/
private theorem Lpnorm_conv_pos (hp : p ≠ 0) (hB : (B₁ ∩ B₂).Nonempty) (hA : A.Nonempty) :
    0 < ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] ^ p :=
  by
  rw [wLpnorm_pow_eq_sum]
  refine'
    sum_pos' (fun x _ => smul_nonneg zero_le' <| by positivity)
      ...
      have  := hA.card_pos
        ...
        ·
          have
                ...
                :=
                mul_nonneg by positivity fun _ => NNReal.coe_nonneg _
        ...
        suffices
            ...
            ∑
              ...
              ∑
                s
                ,
                𝟭 univ.filter fun s => M ^ 2 ≤ g s s * g s
                  *
                  2 * ∑ x , μ B₁ ○ μ B₂ x * 𝟭 A ○ 𝟭 A x ^ p * f x
            by
              obtain ⟨ s , - , hs ⟩ := exists_lt_of_sum_lt this
                ...
                ·
                  simp
                    ...
                    cases
                      hs.not_le
                        mul_nonneg sum_nonneg fun x _ => mul_nonneg this _ <| by positivity
                          <|
                          by positivity
                have
          ...
          rw
              [
                ← @ sum_filter_ne_zero _ _ Filter _ _
                  ...
                  Finset.filter_comm
                  ,
                  filter_true_of_mem fun s hs => h s mem_filter . 1 hs . 2
                  ,
                  ← sum_filter_ne_zero
                ]
            refine'
              lt_mul_of_one_lt_left
                sum_pos fun s hs => h _ mem_filter . 1 hs . 2 . trans_lt' <| by positivity _
                  one_lt_two
...
noncomputable def s (p : ℝ≥0) (ε : ℝ) (B₁ B₂ A : Finset G) : Finset G :=
  univ.filterₓ fun x => (1 - ε) * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] < (𝟭 A ○ 𝟭 A) x

@[simp]
theorem mem_s {p : ℝ≥0} {ε : ℝ} {B₁ B₂ A : Finset G} {x : G} :
    x ∈ s p ε B₁ B₂ A ↔ (1 - ε) * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] < (𝟭 A ○ 𝟭 A) x := by simp [s]

/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (A₁ «expr ⊆ » B₁) -/
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (A₂ «expr ⊆ » B₂) -/
--TODO: When `1 < ε`, the result is trivial since `S = univ`.
theorem sifting (B₁ B₂ : Finset G) (hε : 0 < ε) (hε₁ : ε ≤ 1) (hδ : 0 < δ) (hp : Even p)
...
theorem sifting_cor (hε : 0 < ε) (hε₁ : ε ≤ 1) (hδ : 0 < δ) (hp : Even p) (hp₂ : 2 ≤ p)
