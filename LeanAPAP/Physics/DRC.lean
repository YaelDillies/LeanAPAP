...
private lemma lemma_0 (p : ℕ) (B₁ B₂ A : Finset G) (f : G → ℝ) :
  ...
  simp_rw [mul_assoc]
  simp only [l2inner_eq_sum, IsROrC.conj_to_real, mul_sum, sum_mul, smul_sum, @sum_comm _ _ (Fin p → G), sum_dconv_mul, dconv_apply_sub, Fintype.sum_pow, map_indicate]
  congr with b₁
  congr with b₂
  refine' Fintype.sum_equiv (Equiv.neg $ Fin p → G) _ _ λ s ↦ _
  rw [←smul_mul_assoc, ←smul_mul_smul, card_smul_mu_apply, card_smul_mu_apply, indicate_inter_apply, indicate_inter_apply, mul_mul_mul_comm, prod_mul_distrib]
...
private lemma sum_c (p : ℕ) (B A : Finset G) : ∑ s, (B ∩ c p A s).card = A.card ^ p * B.card := by
  simp only [card_eq_sum_indicate, indicate_inter_apply, c, indicate_inf_apply, mul_sum, sum_mul, sum_pow', @sum_comm _ G, Fintype.piFinset_univ, ←translate_indicate, translate_apply]
  congr with x
...
private lemma lpNorm_conv_pos (hp : p ≠ 0) (hB : (B₁ ∩ B₂).Nonempty) (hA : A.Nonempty) :
  ...
  refine' sum_pos' (λ x _ ↦ smul_nonneg zero_le' $ by positivity)
      ⟨0, mem_univ _, smul_pos _ $ pow_pos _ _⟩
  · refine' lt_of_le_of_ne' (Pi.le_def.1 (dconv_nonneg mu_nonneg mu_nonneg) _) _
    rwa [←Function.mem_support, support_dconv, support_mu, support_mu, ←coe_sub, mem_coe, zero_mem_sub_iff, not_disjoint_iff_nonempty_inter] <;>
      exact mu_nonneg
...
lemma drc (hp₂ : 2 ≤ p) (f : G → ℝ≥0) (hf : ∃ x, x ∈ B₁ - B₂ ∧ x ∈ A - A ∧ x ∈ f.support)
  ...
  · have : 0 ≤ μ_[ℝ] B₁ ○ μ B₂ * 𝟭 A ○ 𝟭 A ^ p * (↑) ∘ f := sorry -- positivity
    convert Fintype.sum_pos $ this.gt_iff_ne.2 $ support_nonempty_iff.1 _
  ...
  suffices : ∑ s, ⟪𝟭_[ℝ] (A₁ s) ○ 𝟭 (A₂ s), (↑) ∘ f⟫_[ℝ] * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] ^ p
    < ∑ s, 𝟭 (univ.filter λ s ↦ M ^ 2 ≤ g s) s * g s * 2 *
  ...
  · obtain ⟨s, -, hs⟩ := exists_lt_of_sum_lt this
    ...
    · simp only [MulZeroClass.zero_mul, l2inner_eq_sum, Function.comp_apply, IsROrC.inner_apply,
      ...
      have : 0 ≤ 𝟭_[ℝ] (A₁ s) ○ 𝟭 (A₂ s) := by sorry -- positivity
  ...
  · rw [←@sum_filter_ne_zero _ _ Filter _ _, Finset.filter_comm,
    ...
    refine' lt_mul_of_one_lt_left
      (sum_pos λ s hs ↦ h _ mem_filter.1 hs.2.trans_lt' $ by positivity _) one_lt_two
 ...
 :=
  ...
  rw
    [
      sum_cast_c, sum_cast_c, sqrt_mul', sqrt_mul', mul_mul_mul_comm sqrt _, mul_self_sqrt, ←mul_assoc, hM_def, div_mul_cancel, ←sqrt_mul, mul_assoc, mul_self_sqrt, hgB, mul_right_comm, mul_assoc
      ]
...
lemma sifting (B₁ B₂ : Finset G) (hε : 0 < ε) (hε₁ : ε ≤ 1) (hδ : 0 < δ) (hp : Even p)
    ...
    ∃ (A₁ : _) (_ : A₁ ⊆ B₁) (A₂ : _) (_ : A₂ ⊆ B₂), 1 - δ ≤ ∑ x in s p ε B₁ B₂ A, (μ A₁ ○ μ A₂) x ∧
        (4 : ℝ)⁻¹ * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] ^ (2 * p) / A.card ^ (2 * p) ≤
            A₁.card / B₁.card ∧
          (4 : ℝ)⁻¹ * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] ^ (2 * p) / A.card ^ (2 * p) ≤
            A₂.card / B₂.card := by
  ...
  have aux :
    ∀ (c : Finset G) (r), (4 : ℝ)⁻¹ * ‖𝟭_[ℝ] A ○ 𝟭 A‖_[p, μ B₁ ○ μ B₂] ^ (2 * p) / A.card ^ (2 * p) ≤ c.card / r →
        c.Nonempty := by
    simp_rw [nonempty_iff_ne_empty]
    rintro c r h rfl
    simp [pow_mul', (zero_lt_four' ℝ).not_le, inv_mul_le_iff (zero_lt_four' ℝ), mul_assoc, div_nonpos_iff, mul_nonpos_iff, (pow_pos (lpNorm_conv_pos hp₀.ne' hB hA) 2).not_le] at h
    norm_cast at h
  ...
  simp only [apply_ite (↑), indicate_apply, Nonneg.coe_one, Nonneg.coe_zero, mul_boole, sum_ite_mem, univ_inter, mul_div_right_comm]
  calc
  ...
  · have : 0 ≤ μ_[ℝ] B₁ ○ μ B₂ := dconv_nonneg mu_nonneg mu_nonneg
    simp_rw [←L1norm_dconv mu_nonneg mu_nonneg, L1norm_eq_sum, norm_of_nonneg (this _), sum_mul, mul_pow]
  calc
