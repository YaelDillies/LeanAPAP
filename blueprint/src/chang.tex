...
\end{align*}
Therefore
\[\mathbb{E}_x e^{\Re f(x)}\leq \mathbb{E}_x \prod_{\gamma\in \Gamma} \left( \cosh(\lvert \widehat{f}(\gamma)\rvert)+\Re c_\gamma \gamma(x)\sinh(\lvert \widehat{f}(\gamma))\right).\]
...
Rudin's exponential inequality for $f$ becomes $\E \exp|\Re f| \le 2\exp(\frac p 2) \le (2 * \sqrt e)^p$. Using $\frac{x^p}{p!} \le e^x$ for positive $x$, we get
