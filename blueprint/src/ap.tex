...
Let $\epsilon>0$ and $m\geq 1$. Let $A\subseteq G$ and $f:G\to \bbc$. If $k\geq 256m\epsilon^{-2}$ then the set

\[ \bbe_{a_1,\ldots,a_k\in A} \norm{ \mu_\tup{a}*f - \mu_A*f }_{2m}^{2m} \leq (\epsilon/2)^{2m}\norm{f}_{2m}^{2m}.\] 
\end{lemma}
\begin{proof}\uses{mzi-complex}
...
\begin{lemma}\label{aps_in_translates}
...
\begin{lemma}\label{lots_of_diagonals}Let $A\subseteq G$ and $k\geq 1$ and $L\subseteq A^k$. Then there exists some $\tup{a}\in L$ such that
...
\end{theorem}
\begin{proof}\uses{random_approx_expect, lots_of_diagonals, aps_in_translates}
Test.
\end{proof}
...
\end{theorem}
\begin{proof}\uses{lp_ap}
Test.
\end{proof}


\begin{theorem}\label{linfty_ap_boosted}
