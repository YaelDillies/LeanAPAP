...
\begin{lemma}
  \label{diss_span}
  \uses{dissociated}
  \lean{Finset.exists_subset_addSpan_card_le_of_forall_addDissociated}
  \leanok

  If $A\subseteq G$ contains no dissociated set with $\geq K+1$ elements,
  then there is $A'\subseteq A$ of size $\abs{A'}\le K$ such that
