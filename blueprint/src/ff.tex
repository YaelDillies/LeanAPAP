...
\end{lemma}
\begin{proof}
\uses{ast_to_circ, ast_expand, circ_expand}
...
It remains to use Lemmas~\ref{ast_expand} and \ref{circ_expand} and apply Lemma~\ref{ast_to_circ}, and note that we can pass from the $L^p$ norm to the $L^p(\mu)$ norm losing a factor of $N^{1/p}$. 
\end{proof}
...
\end{proposition}
\begin{proof}
\uses{global_dichotomy,global_drc,unbalancing,ap_in_ff, circ_expand, cor_sifting}
...
Let $q=2\lceil p'+2^8\epsilon^{-2}\log(64/\epsilon)\rceil$. By Corollary~\ref{cor_sifting}, there are $A_1,A_2$, both of density $\geq \alpha^{2q}$ such that
...
\begin{lemma}\label{no3aps_inn_product}
...
\end{theorem}
\begin{proof}
\uses{no3aps_inn_product, di_in_ff}
...
By Lemma~\ref{no3aps_inn_product} the left-hand side is equal to $\lvert V_t\rvert/\lvert A_t\rvert^2$, and therefore
