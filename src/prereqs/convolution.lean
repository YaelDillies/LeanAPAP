...
@[simp] lemma to_conv_symm_eq : (@to_conv α β).symm = of_conv := rfl
@[simp] lemma of_conv_symm_eq : (@of_conv α β).symm = to_conv := rfl
...
variables {α} [is_R_or_C β]

instance : add_comm_group (with_conv α β) := pi.add_comm_group
...
instance : comm_ring (with_conv α β) :=
...
/-- Convolution -/
def function.conv (f g : α → β) : α → β := of_conv (to_conv f * to_conv g)
...
lemma conv_assoc (f g h : α → β) : f ∗ g ∗ h = f ∗ (g ∗ h) := mul_assoc (to_conv f) _ _

@[simp] lemma conv_zero (f : α → β) : f ∗ 0 = 0 := mul_zero (to_conv f)
@[simp] lemma zero_conv (f : α → β) : 0 ∗ f = 0 := zero_mul (to_conv f)

@[simp] lemma conv_neg (f g : α → β) : f ∗ (-g) = -(f ∗ g) := congr_arg of_conv (mul_neg _ _)
@[simp] lemma neg_conv (f g : α → β) : (-f) ∗ g = -(f ∗ g) := congr_arg of_conv (neg_mul _ _)

lemma conv_add (f g h : α → β) : f ∗ (g + h) = f ∗ g + f ∗ h := congr_arg of_conv (mul_add _ _ _)
lemma add_conv (f g h : α → β) : (f + g) ∗ h = f ∗ h + g ∗ h := congr_arg of_conv (add_mul _ _ _)

lemma conv_sub (f g h : α → β) : f ∗ (g - h) = f ∗ g - f ∗ h := congr_arg of_conv (mul_sub _ _ _)
lemma sub_conv (f g h : α → β) : (f - g) ∗ h = f ∗ h - g ∗ h := congr_arg of_conv (sub_mul _ _ _)
...
lemma conv_comm (f g : α → β) : f ∗ g = g ∗ f := mul_comm (to_conv f) _
...
@[simp] lemma dconv_neg (f g : α → β) : f ○ (-g) = -(f ○ g) := by ext : 1; simp [dconv]
@[simp] lemma neg_dconv (f g : α → β) : (-f) ○ g = -(f ○ g) := by ext : 1; simp [dconv]
