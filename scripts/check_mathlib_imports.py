...
for file in files:
    ...
    for imp in header:
        ...
        if (not imp_file.startswith("Mathlib.")) and \
            (not imp_file.startswith("LeanAPAP.Mathlib")):
            raise OSError(f"""
MathlibImportError: A file in LeanAPAP.Mathlib imports a file not in Mathlib or LeanAPAP.Mathlib
\tImporting file: {file}
